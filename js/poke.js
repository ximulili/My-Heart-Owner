...
H.poke = (function(){
  ...
  function bindPanel(){
    ...
    document.getElementById('pokeBigAvatar').onclick=async ()=>{
      ...
      doAction(actions[Math.floor(Math.random()*actions.length)]);
    };
    document.querySelectorAll('#pokeActionsScroll [data-id]').forEach(b=>b.onclick=async (e)=>{
      if(e.target.dataset.del){ e.stopPropagation(); await delAction(e.target.dataset.del); openPanel(); return; }
      const actions=await H.store.getPokeActions();
      doAction(actions.find(x=>x.id===b.dataset.id));
    });
  ...
  function shake(){
    ...
    document.querySelectorAll('#chatContainer .message-wrapper.received .message-avatar:not(.hidden)').forEach(a=>{
      a.classList.remove('avatar-shaking'); void a.offsetWidth; a.classList.add('avatar-shaking');
    });
  }
  async function doAction(a){
    shake();
    const s=await H.store.getSettings();
    H.chat.appendSystem(`${s.meName||'我'} ${a.action} ${s.partnerName||'TA'}${a.emoji||''}`);
    closePanel();
  }
  function addAction(){
